...
app.post('/webhook', express.raw({ type: 'application/json' }), (req, res) => {
  ...
  if (event.type === 'payment_intent.succeeded') {
    ...
    console.log(`Portfolio Booking for Intent ${paymentIntent.id} succeeded!`);
  }
...
app.post('/create-payment-intent', async (req, res) => {
  if (!stripe) {
    ...
    return res.json({ clientSecret: 'mock_secret_site_portfolio_123' });
  }
...
if (require.main === module) {
  ...
  app.listen(PORT, () => {
    console.log(`Site Portfolio listening on port ${PORT}`);
  });
}
